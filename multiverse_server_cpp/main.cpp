...
#if USE_TCP
    if (t == "tcp")
        return TransportSel::Tcp;
#endif
#if USE_UDP
    if (t == "udp")
        return TransportSel::Udp;
#endif
...
int main(int argc, char** argv)
{
    ...
    if (argc == 1)
    {
        ...
        if (env)
        {
            try
            {
                env_sel = parse_transport_single(env);
            }
            catch (...)
            {
            }
    ...
    for (const auto& sp : specs)
    {
        ...
        if (sp.t == TransportSel::Zmq)
        {
            ...
            if (!r)
            {
                std::cerr << "[Server] ZMQ runner not available\n";
                continue;
            }
        ...
        if (sp.t == TransportSel::Tcp)
        {
            ...
            if (!r)
            {
                std::cerr << "[Server] TCP runner not available\n";
                continue;
            }
        ...
        if (sp.t == TransportSel::Udp)
        {
            ...
            if (!r)
            {
                std::cerr << "[Server] UDP runner not available\n";
                continue;
            }
