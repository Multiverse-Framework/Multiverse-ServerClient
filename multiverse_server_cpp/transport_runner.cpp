...
struct ZmqRunner final : IServerRunner
{
#if USE_ZMQ
    std::string endpoint;
    explicit ZmqRunner(std::string ep)
    ...
    void run() override
    {
        std::cerr << "[ERR] ZMQ not compiled in.\n";
    }
#endif
};
...
struct TcpRunner final : IServerRunner
{
    ...
    TcpRunner(std::string h, std::string p)
        : host(std::move(h))
        , port(std::move(p))
    {
    }
    ...
    void run() override
    {
        std::cerr << "[ERR] TCP not compiled in.\n";
    }
#endif
};
...
struct UdpRunner final : IServerRunner
{
#if USE_UDP
    std::string host, port;
    UdpRunner(std::string h, std::string p)
        : host(std::move(h))
        , port(std::move(p))
    {
    }
    ...
    void run() override
    {
        std::cerr << "[ERR] UDP not compiled in.\n";
    }
#endif
};

std::unique_ptr<IServerRunner> make_runner(
    TransportSel sel, const std::string& bind_or_host, const std::string& port_opt)
{
    switch (sel)
    {
#if USE_ZMQ
