...
void UdpServerTransport::listen(const std::string &endpoint)
{
    ...
    if (rc != 0 || !res)
    {
        throw std::runtime_error(
            make_socket_error_str("UdpServerTransport: getaddrinfo(bind) failed for " + endpoint));
    }
    ...
    throw std::runtime_error(
        make_socket_error_str("UdpServerTransport: bind failed on " + endpoint));
}
...
void UdpServerTransport::send(const void *data, size_t len, bool more)
{
    ...
    if (!more)
    {
        if (!rawudp::send_parts_to(sock_, out_parts_, reinterpret_cast<const sockaddr*>(&peer_addr_), peer_len_))
        {
            out_parts_.clear();
            throw std::runtime_error(
                make_socket_error_str("UdpServerTransport: send_parts_to failed"));
        }
        out_parts_.clear();
    }
}
...
void UdpServerTransport::recv(void *data, size_t len)
{
    ...
    if (in_next_ >= in_parts_.size())
    {
        ...
        if (!peer_known_)
        {
            ...
            if (!rawudp::recv_parts_from(
                    sock_, in_parts_,
                    reinterpret_cast<sockaddr *>(&peer_addr_), &peer_len_, 1000))
            {
    ...
    if (frame.size() != len)
    {
        throw std::runtime_error("UdpServerTransport: recv size mismatch (expected " +
