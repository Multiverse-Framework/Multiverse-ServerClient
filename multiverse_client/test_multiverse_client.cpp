...
public:
    ...
    MyConnector(const std::string& world, const std::string& sim, const std::string& host_,
        const std::string& server_port_, const std::string& client_port_, Mode mode, ClientTransportType transport)
        : mode_(mode)
    {
        ...
        {
        case Mode::Sender:
            send_objects = {{"object1", {"position", "quaternion"}}, {"object2", {"position", "quaternion"}},
                {"joint1", {"joint_angular_position"}}};
            receive_objects.clear();
            break;
        case Mode::Receiver:
            send_objects.clear();
            receive_objects = {{"object1", {"position", "quaternion"}}, {"object2", {"position", "quaternion"}},
                {"joint1", {"joint_angular_position"}}};
            break;
        case Mode::Both1:
            send_objects = {{"object3", {"position"}}, {"object4", {"quaternion"}}};
            receive_objects = {
                {"object3", {"quaternion"}}, {"object4", {"position"}}, {"joint2", {"joint_angular_position"}}};
            break;
        case Mode::Both2:
            send_objects = {
                {"object3", {"quaternion"}}, {"object4", {"position"}}, {"joint2", {"joint_angular_position"}}};
        ...
        std::cout << "[Init] mode="
                  << (mode_ == Mode::Sender
                             ...
                             : (mode_ == Mode::Receiver
                                       ? "Receiver"
                                       : (mode_ == Mode::Both1 ? "Both1"
                                                               : (mode_ == Mode::Both2 ? "Both2"
                                                                                       : (mode_ == Mode::ImageSender
                                                                                                 ? "ImageSender"
                                                                                                 : "ImageReceiver")))))
                  << " transport=" << transport_name(transport) << " host=" << host << " server=" << server_port
                  << " client=" << client_port << "\n";
    }
    ...
    void set_send_object(const std::string& obj, const std::string& attr, const double* data, const size_t data_size)
    {
        ...
        {
            LOG_ERROR("Size mismatch for " << obj << "." << attr << " expected=" << ip->second.size()
                                           << " got=" << data_size);
            return;
        }
    ...
    void set_send_object(const std::string& obj, const std::string& attr, const uint8_t* data, const size_t data_size)
    {
...
private:
    ...
    {
        return !send_objects.empty() || !receive_objects.empty();
    }
    ...
    void bind_response_meta_data() override
    {
        ...
        auto accept = [](const Json::Value& root, std::map<std::string, std::set<std::string>>& out) {
            out.clear();
            if (!root.isObject())
                return;
            for (const auto& obj : root.getMemberNames())
            {
                ...
                if (entry.isObject())
                    for (const auto& attr : entry.getMemberNames())
                        attrs.insert(attr);
                else if (entry.isArray())
                    for (const auto& v : entry)
                        if (v.isString())
                            attrs.insert(v.asString());
                if (!attrs.empty())
                    out[obj] = std::move(attrs);
            }
        };
    ...
    void init_send_and_receive_data() override
    {
        ...
        const size_t send_double_needed =
            std::accumulate(send_objects.begin(), send_objects.end(), size_t(0), [](size_t sum, const auto& so) {
        ...
        const size_t recv_double_needed =
            std::accumulate(receive_objects.begin(), receive_objects.end(), size_t(0), [](size_t sum, const auto& ro) {
    ...
    void bind_send_data() override
    {
        ...
        if (mode_ == Mode::Sender || mode_ == Mode::Both1 || mode_ == Mode::Both2 || mode_ == Mode::ImageSender)
        {
            ...
            {
                set_send_object("image1", "rgb_3840_2160", rgb_3840_2160, 3840 * 2160 * 3);
            }
...
int main(int argc, char** argv)
{
    ...
    auto mode = (args.mode == "sender"
                     ? MyConnector::Mode::Sender
                     : (args.mode == "receiver"
                               ? MyConnector::Mode::Receiver
                               : (args.mode == "both1"
                                         ? MyConnector::Mode::Both1
                                         : (args.mode == "both2" ? MyConnector::Mode::Both2
                                                                 : (args.mode == "imagesender"
                                                                           ? MyConnector::Mode::ImageSender
                                                                           : (args.mode == "imagereceiver"
                                                                                     ? MyConnector::Mode::ImageReceiver
                                                                                     : MyConnector::Mode::Sender))))));

    ClientTransportType tt = ClientTransportType::Tcp;
#if USE_ZMQ
    if (args.transport == "zmq")
        tt = ClientTransportType::Zmq;
#endif
#if USE_TCP
    if (args.transport == "tcp")
        tt = ClientTransportType::Tcp;
#endif
#if USE_UDP
    if (args.transport == "udp")
        tt = ClientTransportType::Udp;
#endif

    std::cout << "[App] mode=" << args.mode << " transport=" << args.transport << " host=" << args.host
              << " server=" << args.server << " client=" << args.client << " world=" << args.world
              << " sim=" << args.sim << "\n";
