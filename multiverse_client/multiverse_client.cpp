...
// Initialize Winsock on Windows (required for TCP/UDP transports)
#if defined(_WIN32) && (USE_TCP || USE_UDP)
...
void MultiverseClient::set_transport(ClientTransportType transport_type, IClientTransport* transport)
{
...
void MultiverseClient::connect_to_server()
{
    if (!transport_)
    {
        printf("[Client %s] Transport not initialized.\n", client_port.c_str());
        return;
    }
    ...
    {
        printf("[Client %s] Client is shutting down, skipping connect_to_server.\n", client_port.c_str());
        return;
    }

    auto current_flag = flag.load();
    if (current_flag == EMultiverseClientState::ReceiveData ||
        current_flag == EMultiverseClientState::ReceiveResponseMetaData)
    {
        sleep_ms(1000);
    }
    ...
    try
    {
        ...
        printf("[Client %s] Received response %s from %s.\n", client_port.c_str(), receive_socket_addr.c_str(),
            server_socket_addr.c_str());
    }
    catch (const std::exception& e)
    {
        ShutdownManager::request_shutdown();
        printf("[Client %s] %s, prepares to disconnect from server socket %s.", client_port.c_str(), e.what(),
            server_socket_addr.c_str());
    }
    ...
    {
        printf("[Client %s] Mismatched response from server broker: expected %s, got %s. Disconnecting.\n",
            client_port.c_str(), socket_addr.c_str(), receive_socket_addr.c_str());
...
void MultiverseClient::connect(
    const std::string& in_host, const std::string& in_server_port, const std::string& in_client_port)
{
    host = in_host;
...
void MultiverseClient::run()
{
    ...
    while (!ShutdownManager::is_shutdown())
    {
        ...
        mv_log("[Client %s] State -> %s (socket: %s) %d", client_port.c_str(), state_name, socket_addr.c_str(),
        ...
        case EMultiverseClientState::InitSendAndReceiveData:
            printf("[Client %s] Initializing send and receive data buffers.\n", client_port.c_str());
            wait_for_connect_to_server_thread_finish();
            printf("[Client %s] Connected to server %s.\n", client_port.c_str(), socket_addr.c_str());
            wait_for_meta_data_thread_finish();
            printf(
                "[Client %s] Meta data exchange completed with server %s.\n", client_port.c_str(), socket_addr.c_str());
            clean_up();
            printf("[Client %s] Cleaned up previous data.\n", client_port.c_str());
            init_send_and_receive_data();

            printf("[Client %s] Starting the communication (send: [%zu - %zu - %zu], receive: [%zu - %zu - %zu]).\n",
                client_port.c_str(), send_buffer.buffer_double.size, send_buffer.buffer_uint8_t.size,
                send_buffer.buffer_uint16_t.size, receive_buffer.buffer_double.size, receive_buffer.buffer_uint8_t.size,
        ...
        current_flag2 != EMultiverseClientState::ReceiveData)
    {
        ...
        printf("[Client %s] Closing the socket %s %s\n", client_port.c_str(), socket_addr.c_str(), state_name);
...
void MultiverseClient::send_send_data()
{
    ...
    if (message_spec_int > 2)
    {
        ...
        size_t remain = (send_buffer.buffer_double.size > 0) + (send_buffer.buffer_uint8_t.size > 0) +
                        (send_buffer.buffer_uint16_t.size > 0);
        ...
        if (send_buffer.buffer_double.size > 0)
        {
            bool last = (--remain == 0);
            transport_->send(
                send_buffer.buffer_double.data, send_buffer.buffer_double.size * sizeof(double), send_more(last));
        }
        if (send_buffer.buffer_uint8_t.size > 0)
        {
            bool last = (--remain == 0);
            transport_->send(
                send_buffer.buffer_uint8_t.data, send_buffer.buffer_uint8_t.size * sizeof(uint8_t), send_more(last));
        }
        if (send_buffer.buffer_uint16_t.size > 0)
        {
            bool last = (--remain == 0);
            transport_->send(
                send_buffer.buffer_uint16_t.data, send_buffer.buffer_uint16_t.size * sizeof(uint16_t), send_more(last));
        }
    }
}
...
void MultiverseClient::receive_data()
{
    ...
    {
        printf("[Client %s] The socket %s from the server has been closed. Reconnecting immediately...\n",
            client_port.c_str(), socket_addr.c_str());

        // Disconnect the dead transport
        try
        {
            transport_->disconnect();
        }
        ...
        // Fast reconnect: skip broker, go directly to StartConnection.
        // The server worker is already waiting for accept() on our port.
        // The run() loop will immediately see StartConnection and reconnect.
        flag = EMultiverseClientState::StartConnection;
        return;
    }
    if (message_spec_int == 0)
    {
        printf("[Client %s] The socket %s from the server has received a shutdown signal.\n", client_port.c_str(),
            socket_addr.c_str());
        ShutdownManager::request_shutdown();
        return;
    }
    ...
    {
        printf("[Client %s] The socket %s from the server is receiving meta data.\n", client_port.c_str(),
            socket_addr.c_str());
        // meta-data string
    ...
    else if (message_spec_int >= 2)
    {
        ...
        if (message_spec_int == 3)
        {
            ...
            else if (receive_buffer.buffer_double.size == 0 && receive_buffer.buffer_uint8_t.size > 0 &&
                     receive_buffer.buffer_uint16_t.size == 0)
            {
                transport_->recv(
                    receive_buffer.buffer_uint8_t.data, receive_buffer.buffer_uint8_t.size * sizeof(uint8_t));
            }
            else if (receive_buffer.buffer_double.size == 0 && receive_buffer.buffer_uint8_t.size == 0 &&
                     receive_buffer.buffer_uint16_t.size > 0)
            {
                transport_->recv(
                    receive_buffer.buffer_uint16_t.data, receive_buffer.buffer_uint16_t.size * sizeof(uint16_t));
            }
            else
            {
...
void MultiverseClient::check_response_meta_data()
{
    if (ShutdownManager::is_shutdown())
    {
        printf("[Client %s] The client is shutting down, skipping check_response_meta_data.\n", client_port.c_str());
        flag = EMultiverseClientState::BindResponseMetaData;
    }
...
{
    std::map<std::string, std::map<std::string, size_t>> req = {{"send", {{"double", 0}, {"uint8", 0}, {"uint16", 0}}},
        {"receive", {{"double", 0}, {"uint8", 0}, {"uint16", 0}}}};
    compute_request_buffer_sizes(req["send"], req["receive"]);

    std::map<std::string, std::map<std::string, size_t>> rsp = {{"send", {{"double", 0}, {"uint8", 0}, {"uint16", 0}}},
        {"receive", {{"double", 0}, {"uint8", 0}, {"uint16", 0}}}};
    compute_response_buffer_sizes(rsp["send"], rsp["receive"]);

    if ((int)req["receive"]["double"] != -1 && (int)req["receive"]["uint8"] != -1 &&
        (int)req["receive"]["uint16"] != -1 &&
        (req["send"]["double"] != rsp["send"]["double"] || req["send"]["uint8"] != rsp["send"]["uint8"] ||
            req["send"]["uint16"] != rsp["send"]["uint16"] || req["receive"]["double"] != rsp["receive"]["double"] ||
            req["receive"]["uint8"] != rsp["receive"]["uint8"] || req["receive"]["uint16"] != rsp["receive"]["uint16"]))
    {

        printf("[Client %s] Failed to initialize the buffers %s: send_buffer_size(server = [%zu - %zu - %zu], client = "
               "[%zu - %zu - %zu]), "
               "receive_buffer_size(server = [%zu - %zu - %zu], client = [%zu - %zu - %zu]).\n",
            client_port.c_str(), socket_addr.c_str(), req["send"]["double"], req["send"]["uint8"],
            req["send"]["uint16"], rsp["send"]["double"], rsp["send"]["uint8"], rsp["send"]["uint16"],
            req["receive"]["double"], req["receive"]["uint8"], req["receive"]["uint16"], rsp["receive"]["double"],
            rsp["receive"]["uint8"], rsp["receive"]["uint16"]);
        return false;
    }
...
{
#define SAFE_CALLOC(count, type) ((count) > 0 ? (type*)calloc((count), sizeof(type)) : nullptr)
...
{
    const auto current_flag = flag.load();
    if (ShutdownManager::is_shutdown() || current_flag == EMultiverseClientState::None)
        return false;
    ...
    {
        printf("[Client %s] Communication error: %s. Reconnecting...\n", client_port.c_str(), e.what());
        try
        {
            transport_->disconnect();
        }
        ...
        // Fast reconnect: skip broker, connect directly on next tick.
        // Server worker is already waiting for accept() on our port.
        flag = EMultiverseClientState::StartConnection;
        return false;
    }
    catch (...)
    {
        printf("[Client %s] Unknown communication error. Reconnecting...\n", client_port.c_str());
        try
        {
            transport_->disconnect();
        }
...
void MultiverseClient::disconnect()
{
    if (!transport_)
    {
        printf("[Client %s] The client transport is not initialized.\n", client_port.c_str());
        return;
    }
    ShutdownManager::request_shutdown();
