...
inline uint32_t htole32_custom(uint32_t host_32bits)
{
    ...
    if (*reinterpret_cast<const uint8_t*>(&test) == 1)
        ...
        return ((host_32bits & 0xFF000000u) >> 24) | ((host_32bits & 0x00FF0000u) >> 8) |
               ((host_32bits & 0x0000FF00u) << 8) | ((host_32bits & 0x000000FFu) << 24);
#endif
}
...
inline bool encode_parts(const std::vector<std::string>& parts, std::vector<uint8_t>& out)
{
    ...
    for (uint32_t i = 0; i < n; ++i)
    {
        ...
        if (sz > 0)
        {
            // mv_log("[rawudp] Encoding part %u data (hex): %s", i, hex_dump(s).c_str());
...
inline bool decode_parts(const uint8_t* buf, size_t len, std::vector<std::string>& out)
{
    ...
    for (uint32_t i = 0; i < n; ++i)
    {
        ...
        if (p + sz > e)
        {
            mv_log("[rawudp] Truncated data for part %u (need %u bytes, have %zu).", i, sz, (size_t)(e - p));
            return false;
        }
        ...
        if (sz > 0)
        {
            // mv_log("[rawudp] Read data for part %u (hex): %s", i, hex_dump(s).c_str());
        }
        else
        {
    ...
    mv_log("[rawudp] Successfully received all %u parts. Used %zu/%zu bytes", n, (size_t)(p - buf), len);
    return true;
}
...
inline bool send_parts(socket_t fd, const std::vector<std::string>& parts)
{
    ...
    if (!encode_parts(parts, pkt))
        return false;

    mv_log("[rawudp %d] Sending %zu byte packet", fd, pkt.size());
    ...
    mv_log("[rawudp %d] Successfully sent %zd bytes", fd, (ssize_t)w);
    return true;
}
...
inline bool recv_parts(socket_t fd, std::vector<std::string>& out, int timeout_ms = 5000)
{
    ...
    mv_log("[rawudp %d] Received %zd bytes", fd, (ssize_t)n);
    return decode_parts(buf.data(), (size_t)n, out);
}

// -------- Unconnected UDP send/recv --------
inline bool send_parts_to(
    socket_t fd, const std::vector<std::string>& parts, const struct sockaddr* dest, socklen_t dest_len)
{
    std::vector<uint8_t> pkt;
    if (!encode_parts(parts, pkt))
        return false;
...
inline bool recv_parts_from(socket_t fd, std::vector<std::string>& out, struct sockaddr* from = nullptr,
    socklen_t* from_len = nullptr, int timeout_ms = 1000)
{
    std::vector<uint8_t> buf(MAX_UDP_PAYLOAD);
